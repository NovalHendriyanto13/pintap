...
export class UserController {
  ...
  async update(@Request() req): Promise<any> {
    const { id } = req.params;
    const { numberOfAffectedRows, updatedUser } = await this.userService.update(id, req.body);
  ...
  async delete(@Request() req): Promise<any> {
    const { id } = req.params;
    const { numberOfAffectedRows, updatedUser } = await this.userService.delete(id);
    if (numberOfAffectedRows === 0) {
        throw new NotFoundException('This Post doesn\'t exist');
    }
